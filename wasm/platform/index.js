...
function loadProductInfos() {
  ...
  if (modelCodePlaceholder) {
    ...
    const moonlightVersion =
      window.tizen.application.getAppInfo().version || "Not Available";
    const tizenVersion =
      window.tizen.systeminfo.getCapability(
...
function attachListeners() {
  ...
  const registerMenu = (elementId, view) => {
    $(`#${elementId}`).on("click", () => {
      if (view.isActive()) Navigation.pop();
      else Navigation.push(view);
    });
  };
...
function sendEscapeToHost() {
  //FIXME: workaround to send escape key to host
...
function updateMacAddress(host) {
  //FIXME(?) : needed to correctly set the stored mac address (indexedDB)
  getData("hosts", function (previousValue) {
    ...
    if (host.macAddress != "00:00:00:00:00:00") {
      if (
        hosts[currentHostUID] &&
        hosts[currentHostUID].macAddress != host.macAddress
      ) {
        console.log(
          "Updated MAC address for host " +
            host.hostname +
            " from " +
            hosts[currentHostUID].macAddress +
            " to " +
            host.macAddress
        );
        saveHosts();
  ...
  nvhttpHost.pollServer(function (ret) {
    if (!nvhttpHost.online) {
      snackbarLog(
        "Failed to connect to " +
          nvhttpHost.hostname +
          "! Ensure Sunshine is running on your host PC or GameStream is enabled in GeForce Experience SHIELD settings."
      );
      console.error(
        "%c[index.js]",
        "color: green;",
    ...
    $("#pairingDialogText").html(
      "Please enter the following PIN on the target PC:  " +
        randomNumber +
        "<br><br>If your host PC is running Sunshine, navigate to the Sunshine web UI to enter the PIN.<br>Alternatively, navigate to the GeForce Experience (NVIDIA GPUs only) to enter the PIN.<br><br>This dialog will close once the pairing is complete."
    );
    pairingDialog.close();
...
function addHostToGrid(host, ismDNSDiscovered) {
  ...
  var options = [
    // host settings dialog options, used an array to make it easier to add more options
    {
      text: "Wake PC (WOL)",
      id: "wake-" + host.hostname,
      action: function () {
        host.sendWOL();
      },
    },
    ...
    {
      text: "Refresh box art",
      id: "refreshBoxArt-" + host.hostname,
      action: function () {
        host.purgeBoxArt();
      },
    },
...
function showRestartMoonlightDialog() {
  var restartMoonlightDialog = document.querySelector(
...
function showTerminateMoonlightDialog() {
  var terminateMoonlightDialog = document.querySelector(
  ...
  host.refreshServerInfo().then(function (ret) {
    host.getAppById(appID).then(function (appToStart) {
      ...
      const audioSyncEnabled = $("#audioSyncSwitch")
        ...
        ? 1
        : 0;
      const hdrEnabled = $("#hdrSwitch").parent().hasClass("is-checked")
        ? 1
        : 0;
      console.log(
        "%c[index.js, startGame]",
  ...
  let cb = function () {
    try {
      ...
      transaction.onerror = function (e) {
        console.error(
          'Error reading data at key: "' + key + '" from IndexDB: ' + e
        );
        callCb(key, value, callbackFunction);
      };
    } catch (e) {
...
function saveFramePacing() {
  setTimeout(function () {
    ...
    console.log(
      ...
      "color: green;",
      "Saving frame pacing state : " + chosenFramePacing
    );
...
function updateDefaultBitrate() {
  ...
  if (res === "858:480") {
    ...
    if (frameRate === "30") {
      ...
      // 2160p (4K), 120 FPS
      $("#bitrateSlider")[0].MaterialSlider.change("160");
    }
  } else {
...
function loadUserDataCb() {
    ...
    if (previousValue.codecVideo != null) {
      $(".codecVideoMenu li").each(function () {
        if ($(this).data("value") === previousValue.codecVideo) {
          $("#selectCodecVideo")
            .text($(this).text())
            .data("value", previousValue.codecVideo);
        }
      });
    }
  });
  ...
  getData("bitrate", function (previousValue) {
    $("#bitrateSlider")[0].MaterialSlider.change(
      previousValue.bitrate != null ? previousValue.bitrate : "20"
    );
    updateBitrateField();
  });
}
...
window.addEventListener("gamepadconnected", function (event) {
  ...
  if (connectedGamepad.vibrationActuator) {
    // Check if the gamepad supports rumble
